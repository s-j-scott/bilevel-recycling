...
def tensor_up_info(info):
    for key, item in info.items():
        if isinstance(item, list):
            first = item[0]
            if torch.is_tensor(first):
                if first.numel() == 1:
                    info[key] = torch.cat(item)

#Gradient descent
def gradient_descent( fun, x0, max_its=10000, grad_tol=1e-12, verbose=False,
                     desc='', store_iters=False, full_info = True, step_size=1e-2, constraint_fun = None):
    """
    ...
    step_next = step_size # Initial step length for backtracking algorithm for first iteration of gradient descent
    if full_info:
        info = { # Dictionary for extra information about GD iterations
            't_grad_computation':[], # array of length of times for each gradient computation
            'grad_norms':[], # array of the gradient norms at each iteration
            'step_lengths': [],# array of step lengths used for each iteration of algorithm
            'backtrack_its':[], # array of number of backtracking iterations used for each step length
            't_step_length':[], # array of length of time for each step length computation
            't_iteration_total':[], # array of length of overall time for each iteration
            'fun_evals':[], # array of length of evaluation of cost function
    ...
    if store_iters: info['all_iters'] = [x0.detach().clone().cpu()] # History of considered points
    ...
    for it in range(max_its):
        ...
        if full_info:
            info['t_grad_computation'].append(t_grad)
            info['grad_norms'].append ( grad_norm.detach().cpu().item())
        ...
        if grad_norm <= grad_tol:
            ...
            if full_info: 
                info['t_iteration_total'].append(time.time() - t_iteration_start)
                info['fun_evals'].append(fun_new.detach().cpu().item()) # Function evaluation at current iteration
            else: 
                info['grad_norms'] = grad_norm.detach().cpu().item()
            pbar.close()
            tensor_up_info(info)
            return x, info
        ...
        t_tmp = time.time()
        step_length, step_next, backtrack_satisfied, backtrack_it, x_new, fun_new, fun_old = backtracking(fun, grad_x, x,
                                                                            search_dir=-grad_x, 
                                                                            step_size=step_next, 
                                                                            fun_eval = fun_new,
                                                                            verbose=False
                                                                            )
        # step_length, backtrack_it, fun_old, backtrack_satisfied = 1e-2, 0,0,1
        # x_new = x + step_length * grad_x
        
        t_step_length = time.time() - t_tmp
        
        if full_info:
            info['step_lengths'].append(step_length)
            info['backtrack_its'].append(backtrack_it)
            info['t_step_length'].append( t_step_length)
            info['fun_evals'].append( fun_old.detach().cpu().item()) # We know that x_k has been accepted so store the evaluation. Possible we reject the determined step length
        ...
        if full_info: 
            info['t_iteration_total'].append(time.time() - t_iteration_start)
        if store_iters: info['all_iters'].append(x.detach().clone().cpu())
    ...
    info['t_total'] = time.time() - t_start_loop
    info['convg_flag']  = 1
    if not full_info: info['grad_norms'] = grad_norm.detach().cpu().item()
...
def backtracking(fun, grad_s, s, search_dir, step_size=1e-2, fun_eval=None, verbose=False):
    """
    ...
    while step_size > 1e-14:
        ...
        if fun_snew < fun_s - discount*step_size*grad_search_inner:
            ...
            if verbose:
                print(f'Backtrack termianted after {it} iterations.')
...
def bfgs(fun, x0, max_its=10000, grad_tol=1e-12, verbose=False, 
    ...
    if store_iters: info['all_iters'] = [x0.detach().cpu()] # History of considered points
...
def MINRES(A, b, x0=None, tol=1e-6, max_its=10000, verbose=False, full_info = True,
    ...
    for it in range(max_its):
        ...
        rnorm = torch.norm(A(x)-b) # Residual
        
        #=== Stopping criteria        
        if full_info: 
            info['basis_coefs'][it] = update_coef
            info['r_norms'][it+1] = rnorm.cpu()
            info['xs'].append(x.clone().cpu())
        if store_basis_vecs:
        ...
        if verbose:
            # Update progress bar
            pbar_info = f'Residual:{rnorm.cpu():.5f}'
            pbar.postfix = (pbar_info)
            pbar.update()
