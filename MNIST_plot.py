...
stop_it_none, ul_cost_none, cost_none, all_hg_err_none = (
    state["stop_it_none"],
    state["ul_cost_none"],
    state["cost_none"],
    state["all_hg_err_none"],
)

baseline = {
    "none": {
        "stop_it": state["stop_it_none"],
...
pltt.plot_data(
    torch.log10(diff_info["dif_hess"] / diff_info["norm_hess"]),
    ylabel="log10 $H^{(i)}$ relative diff",
    xlabel="Linear system number",
)
pltt.plot_data(
    torch.log10(diff_info["dif_rhs"] / diff_info["norm_rhs"]),
    ylabel="log10 $g^{(i)}$ relative diff",
    xlabel="Linear system number",
)
pltt.plot_data(
    torch.log10(diff_info["dif_w"] / diff_info["norm_w"]),
    ylabel="log10 $w^{(i)}$ relative diff",
    xlabel="Linear system number",
)
pltt.plot_data(
    torch.log10(diff_info["dif_recon"] / diff_info["norm_recon"]),
...
pltt.plot_timings(
    {
        "t_total": bl_info["ul_itn_times"],
        "t_hess": bl_info["t_hess"],
        "t_ll": bl_info["t_total_lls"],
    },
...
pltt.compare_performance(
    baseline["none"],
    select_results(["Ritz-S", "Ritz-M", "Ritz-L"], results),
...
pltt.compare_performance(
    baseline["none"],
    select_results(["HRitz-S", "HRitz-M", "HRitz-L"], results),
...
pltt.compare_performance(
    baseline["none"],
    select_results(
        ["RGen-S(L)", "RGen-M(L)", "RGen-L(L)", "RGen-L(L)-NSC"], results
    ),
...
pltt.compare_performance(
    baseline["none"],
    select_results(
        ["RGen-S(R)", "RGen-M(R)", "RGen-L(R)", "RGen-L(R)-NSC"], results
    ),
...
pltt.compare_performance(
    baseline["none"],
    select_results(
        ["RGen-S(M)", "RGen-M(M)", "RGen-L(M)", "RGen-L(M)-NSC"], results
    ),
...
pltt.compare_performance(
    baseline["none"],
    select_results(["Ritz-S", "RGen-L(R)", "RGen-L(R)-NSC"], results),
...
pltt.compare_performance(
    baseline["none"],
    select_results(["Ritz-S Outer", "Ritz-S Inner"], results_io),
    save=FIGURES_DIR + "io_compare_ritz.png",
    quantity_keys=quantity_keys,
    ylabels=ylabels,
)
...
pltt.compare_performance(
    baseline["none"],
    select_results(["RGen-L(R) Outer", "RGen-L(R) Inner"], results_io),
    save=FIGURES_DIR + "io_compare_gritz.png",
    quantity_keys=quantity_keys,
    ylabels=ylabels,
)
...
pltt.compare_performance(
    baseline["none"],
    select_results(["RGen-L(R)-NSC Outer", "RGen-L(R)-NSC Inner"], results_io),
    save=FIGURES_DIR + "io_compare_gritznew.png",
    quantity_keys=quantity_keys,
    ylabels=ylabels,
)
...
for ind in [14, 39, 59, 94]:
    state_hg_tmp = torch.load(
        LOAD_DIR + "hg_err_comp_system" + str(ind), weights_only=False
    )
    pltt.plot_comparison_of_hg_err_approx(
        state_hg_tmp["true"],
        state_hg_tmp["approx"],
        state_hg_tmp["cheat"],
...
results_dim_baseline = {
    "stop_its": baseline["none"]["stop_it"].sum()
    * torch.ones(len(state["rec_dims"])),
    "hg_rerr_mean": baseline["none"]["hg_rerr"].mean()
    * torch.ones(len(state["rec_dims"])),
    "hg_rerr_max": baseline["none"]["hg_rerr"].max()
    * torch.ones(len(state["rec_dims"])),
...
results_dim = {
    ...
    "RGen-L(R)": {
        "flops": state["c_gritz"].sum(axis=1),
    ...
    "RGen-L(R)-NSC": {
        "flops": state["c_gritz2"].sum(axis=1),
